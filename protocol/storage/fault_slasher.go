...
func (sfm *FaultSlasher) Slash(ctx context.Context, currentHeight *types.BlockHeight) error {
	...
	for lateMinerActor, state := range *lms {
		...
		laterMinerActorAddr, err := address.NewFromString(lateMinerActor)
		if err != nil {
			return errors.Wrap(err, "could not create minerAddr string")
		}

		if laterMinerActorAddr == myMinerActorAddr {
			sfm.log.Warningf("skip slashing self %s", laterMinerActorAddr.String())
			continue
		}

		// add slash message to message pull w/o broadcasting
		sfm.log.Debugf("Slashing %s with state %d\n", laterMinerActorAddr, state)

		_, err = sfm.outbox.Send(ctx, myWorkerAddr, laterMinerActorAddr, types.ZeroAttoFIL, sfm.gasPrice,
